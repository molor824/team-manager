...
public class ProjectController {
    ...
    @PostMapping
    public ProjectResponseDto createProject(@RequestBody ProjectDto projectDto) throws StatusException {
        return new ProjectResponseDto(projectService.createProject(projectDto));
    }

    @GetMapping("/{id}")
    public ProjectResponseDto getProjectById(@PathVariable Long id) throws StatusException {
        return new ProjectResponseDto(projectService.getProjectById(id));
    }

    @GetMapping
    public List<ProjectResponseDto> getAllProjects() throws StatusException {
        return projectService.getAllProjects().stream().map(ProjectResponseDto::new).collect(Collectors.toList());
    }

    @PutMapping("/{id}")
    public ProjectResponseDto updateProject(@PathVariable Long id, @RequestBody ProjectDto projectDto) throws StatusException {
        return new ProjectResponseDto(projectService.updateProject(id, projectDto));
    }

    @DeleteMapping("/{id}")
    public void deleteProject(@PathVariable Long id) throws StatusException {
        projectService.deleteProject(id);
    }

    @DeleteMapping("/{projectId}/member/{memberId}")
    public void leaveProject(@PathVariable Long projectId, @PathVariable Long memberId) throws StatusException {
        projectService.removeUserFromProject(projectId, memberId);
    }

    @GetMapping("/{projectId}/users")
    public List<UserDto> getMembersInProject(@PathVariable Long projectId) throws StatusException {
